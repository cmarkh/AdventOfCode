...
-2,1`
	...
	scans := parse(strings.Split(test, "\n"))
	for i, scan := range scans {
		fmt.Printf("scanner %d: %v\n", i, scan)
	}

	_, offsets := rotateScanner(scans[0], scans[1], 3)
	fmt.Printf("offsets: x: %v, y: %v, z: %v\n", offsets.x, offsets.y, offsets.z)

	fmt.Println()

	if offsets.x != 5 || offsets.y != 2 {
		t.Fail()
	}
}
...
func Test3D0(t *testing.T) {
	...
	success, offsets := rotateScanner(scans[1], scans[3], 12)
...
func Test3D2(t *testing.T) {
	...
	for i, scanner := range scanners {
		...
		fmt.Printf("x: %v, y: %v, z: %v\n", scanner.offsets.x, scanner.offsets.y, scanner.offsets.z)
	}
...
1,2,3`
	...
	rotateScanner(scans[0], scans[1], 3)

}

func TestNegativePermutations(t *testing.T) {
	negatives := negativePermutations(3)
